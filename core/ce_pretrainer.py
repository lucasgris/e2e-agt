...
from core.criterions import CEPretrainWithDiversityLoss, CEPretrainLoss
...
class CEPretrainTrainer(Trainer):
    ...
    class History:
        ...
        def remove_inf_nan(self):
            ...
            self.ce_loss = [
                c for c in self.ce_loss if not np.isnan(c) and not np.isinf(c)
            ]
            self.accuracy = [
        ...
        def average_last_n(self, n=10):
            ...
            return {
                ...
                "ce_loss": np.mean(self.ce_loss[-n:]),
    ...
    def __init__(self, cfg: DictConfig, run_dir: Union[os.PathLike, str]):
        ...
        self.quantize_targets = cfg.quantize_targets

        if str(cfg.criterions.pretrain.name) == "CEPretrainWithDiversityLoss":
            self.criterion = CEPretrainWithDiversityLoss(cfg.criterions.pretrain.config)
        elif str(cfg.criterions.pretrain.name) == "CEPretrainLoss":
            self.criterion = CEPretrainLoss(cfg.criterions.pretrain.config)
        else:
            raise ValueError(
                f"A valid criterion for pretraining must be specified, got {cfg.criterions.pretrain.name}"
            )

        if cfg["model"]["projection"] is None:
            raise ValueError("Projection layer must be specified for pretraining")
    ...
    def _compute_losses(
        ...
        if self.quantize_targets:
            step_loss, ce_loss, diversity_loss = self.criterion(
                encoder_out, labels, perplexity, time_mask_indices
            )
        else:
            step_loss = self.criterion(encoder_out, labels, time_mask_indices)
        ...
        if self.quantize_targets:
            return step_loss, ce_loss, diversity_loss
        else:
            return step_loss
    ...
    def _log_tensorboard_step(
        ...
        tb.log_step("ce_loss", float(step_ce_loss.item()), self.current_update)
        if self.quantize_targets:
            tb.log_step(
                "diversity_loss", float(step_diversity_loss.item()), self.current_update
            )
    ...
    def _log_tensorboard_epoch(self, tb, hist):
        ...
        tb.log_epoch("ce_loss", float(np.mean(hist.ce_loss)), self.current_epoch)
        tb.log_epoch("accuracy", float(np.mean(hist.accuracy)), self.current_epoch)
        if self.quantize_targets:
            tb.log_epoch(
                "diversity_loss",
                float(np.mean(hist.diversity_loss)),
                self.current_epoch,
            )
            tb.log_epoch(
                "perplexity", float(np.mean(hist.perplexity)), self.current_epoch
            )
            ...
            try:
                ...
                with torch.cuda.amp.autocast(enabled=self.use_amp):
                    ...
                    if self.quantize_targets:
                        ...
                        step_loss, step_ce_loss, step_diversity_loss = (
                        ...
                        step_entropy = torch.sum(
                            torch.sum(batch_ppl * log_perplexity, dim=-1)
                        )
                    else:
                        batch_output, batch_target, batch_mask_indices = (
                            output["o"],
                            output["y"],
                            output["mask_indices"],
                        )
                        step_loss = step_ce_loss = self._compute_losses(
                            encoder_out=batch_output,
                            labels=batch_target,
                            time_mask_indices=batch_mask_indices,
                        )
                ...
                if (
                    ...
                    if self.quantize_targets:
                        train_hist.perplexity.append(
                            float(step_ppl.detach().cpu().item())
                        )
                        train_hist.entropy.append(
                            float(step_entropy.detach().cpu().item())
                        )
                        train_hist.diversity_loss.append(
                            float(step_diversity_loss.detach().cpu().item())
                        )
                    train_hist.ce_loss.append(float(step_ce_loss.detach().cpu().item()))
                    ...
                    self._log_tensorboard_step(
                        ...
                        step_ce_loss,
                        step_accuracy,
                        step_diversity_loss if self.quantize_targets else 0,
                        step_ppl if self.quantize_targets else 0,
                        step_entropy if self.quantize_targets else 0,
                    )
                ...
                if self.quantize_targets:
                    log_prog = [
                        ...
                        f"ce_loss: {float(step_ce_loss):.3f}↓",
                ...
                if self.log_interval_steps > 0 and idx % self.log_interval_steps == 0:
                    ...
                    if self.quantize_targets:
                        log_interval_steps = {
                            ...
                            "ce_loss_avg": f"{avg_metrics.get('ce_loss'):.3f}",
    ...
    def _log_images(
        ...
        if len(batch_x[ex_idx].shape) == 3:
            tb.log_image(
                f"features/x", batch_x[ex_idx].transpose(1, 2), self.current_update
            )
            self._log_heatmap(
        ...
        if batch_ffm.nelement() != 0:
            ...
            if heatmap:
                self._log_heatmap(
                    tb,
                    "heatmaps/features/ffm",
                    batch_ffm[ex_idx].transpose(1, 2),
                    self.current_update,
                )
        ...
        if output.get("y", None) is not None:
            tb.log_image(
                f"y",
                output["y"][ex_idx, :, :].squeeze().unsqueeze(0).transpose(1, 2),
                self.current_update,
            )
            if heatmap:
                self._log_heatmap(
                    tb,
                    "heatmaps/y",
                    output["y"][ex_idx, :, :].squeeze().transpose(0, 1),
                    self.current_update,
                )
        ...
        for idx, batch in evaluation_step_it:
            ...
            with torch.no_grad():
                ...
                if self.quantize_targets:
                    ...
                    step_loss, step_ce_loss, step_diversity_loss = self._compute_losses(
                        encoder_out=batch_output,
                        labels=batch_target,
                        perplexity=batch_ppl,
                        time_mask_indices=batch_mask_indices,
            ...
            valid_hist.ce_loss.append(float(step_ce_loss.detach().cpu().item()))
            if self.quantize_targets:
        ...
        for _ in range(1, self.max_epochs + 1):
            ...
            if self.current_epoch > self.max_epochs:
                ...
                if self.quantize_targets:
                    epoch_info = {
                        ...
                        "valid_ce_loss": f"{valid_hist_avg.get('ce_loss'):.3f}",
            ...
            if self.quantize_targets:
                epoch_info = {
                    ...
                    "ce_loss": f"{train_hist_avg.get('ce_loss', 0):.3f}",
                    ...
                    epoch_info.update(
                        {
                            ...
                            "valid_ce_loss": f"{valid_hist_avg.get('ce_loss', 0):.3f}",
        ...
        if self.run_test_at_end:
            ...
            if self.quantize_targets:
                info = {
                    ...
                    "test_ce_loss": f"{test_hist_avg.get('ce_loss', 0):.3f}",
